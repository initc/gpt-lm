...
class BertConfig(object):
    ...
    def __init__(self,
        ...
        self.task_type_size = task_type_size
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.task_embeddings = nn.Embedding(config.task_type_size, config.hidden_size)
    ...
    def forward(self, input_ids, token_type_ids=None, task_type_ids=None, position_ids=None):
        # pdb.set_trace()
        ...
        if not self.fp32_tokentypes:
            ...
            if task_type_ids is not None:
                task_embedding = self.task_embeddings(task_type_ids)
...
class BertSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer("clm_mask", torch.triu(torch.ones(2*n_ctx, 2*n_ctx).byte(), diagonal=1).view(1, 1, 2*n_ctx, 2*n_ctx))

        self.dropout = nn.Dropout(config.attention_probs_dropout_prob)

    def apply_clm_mask(self, scores):
        ns, nt = scores.size(-2), scores.size(-1)
        b = self.clm_mask[:, :, nt-ns:nt, :nt]
        return scores.float().masked_fill(b, float('-inf')).type_as(scores)
    ...
    def forward(self, hidden_states, attention_mask, clm=False, layer_past=None):
        ...
        key_layer = self.transpose_for_scores(mixed_key_layer, k=True)
        value_layer = self.transpose_for_scores(mixed_value_layer)
        if layer_past is not None:
            past_key, past_value = layer_past[0].transpose(-2, -1), layer_past[1]
            key_layer = torch.cat([past_key, key_layer], dim=-1)
            value_layer = torch.cat([past_value, value_layer], dim=-2)
        present = torch.stack((key_layer.transpose(-2, -1), value_layer))
        # print(present.size())
        ...
        if clm:
            attention_scores = self.apply_clm_mask(attention_scores)
        else:
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, clm=False, layer_past=None):
        self_output, present = self.self(input_tensor, attention_mask, clm=clm, layer_past=layer_past)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, clm=False, layer_past=None):
        attention_output, present = self.attention(hidden_states, attention_mask, clm=clm, layer_past=layer_past)
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, clm=False, past=None, output_all_encoded_layers=True, checkpoint_activations=False):
        if past is None:
        ...
        def custom(start, end):
            def custom_forward(*inputs):
                ...
                i = start
                for layer in layers:
                    x_, present = layer(x_, inputs[1], clm=clm, layer_past=past[i])
        ...
        if checkpoint_activations:
            ...
            for i, layer_module in enumerate(self.layer):
                hidden_states, present = layer_module(hidden_states, attention_mask, clm=clm, layer_past=past[i])
                presents.append(present)
                if output_all_encoded_layers:
                    all_encoder_layers.append(hidden_states)
        ...
        if not output_all_encoded_layers or checkpoint_activations:
            all_encoder_layers.append(hidden_states)
        return all_encoder_layers, presents
...
class PreTrainedBertModel(nn.Module):
    ...
    def load_model(cls, model_config,
        ...
        config_file = os.path.join(model_config, CONFIG_NAME)
        config = BertConfig.from_json_file(config_file)
        config.fp32_layernorm = fp32_layernorm
        config.fp32_embedding = fp32_embedding
        config.layernorm_epsilon = layernorm_epsilon
    ...
    @classmethod
    def build_model(cls, pretrained_model_name,
                        fp32_layernorm=False, fp32_embedding=False, layernorm_epsilon=1e-12,
                        fp32_tokentypes=False, *inputs, **kwargs):
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, task_type_ids=None, attention_mask=None, clm=False, past=None, output_all_encoded_layers=True, checkpoint_activations=False):
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids, task_type_ids=task_type_ids, position_ids=position_ids)
        encoded_layers, presents = self.encoder(embedding_output,
                                      extended_attention_mask, clm=clm, past=past,
        ...
        return encoded_layers, pooled_output, presents
...
class BertForPreTraining(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, task_type_ids=None, attention_mask=None, masked_lm_labels=None, next_sentence_label=None, clm=False, past=None, checkpoint_activations=False):
        sequence_output, pooled_output, presents = self.bert(input_ids, token_type_ids, task_type_ids, attention_mask, clm=clm, past=past,
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            return total_loss, presents
        else:
            return prediction_scores, seq_relationship_score, presents
