...
def load_checkpoint(model, optimizer, lr_scheduler, args):
    ...
    model_state = model_sd['sd']
    if args.fix_model_state:
        model_state = convert_model(model_state)
...
def check_checkpoint(model, optimizer, lr_scheduler, args):
    checkpoint_path = args.save
    last_checkpoints = os.path.join(checkpoint_path, "checkpoints-last.pt")
    if os.path.exists(last_checkpoints):
        model_sd = torch.load(last_checkpoints, map_location='cpu')
        total_iters = model_sd['total_iters']
        epoch = model_sd['epoch']
        i = model_sd['mid_epoch_iters']
        model_state = model_sd['sd']
        ...
        print("| continue train with last checkpoints : {}".format(last_checkpoints))
